...
cd ~/el
[ -d buildroot ] && { mv buildroot buildroot.beagleboard; }
[ -d buildroot.beagleboard ] || { echo "no buildroot installed. pls install from example 1 in ch3."; exit 1; }

# Step #2: ok. it is there; list configs and set the defconfig
cd buildroot.beagleboard
make list-defconfigs | grep beagleboard
make beagleboard_64_defconfig
